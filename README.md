...
**ProxyTor Gateway** is a self-hosted Tor + Privoxy proxy gateway with a web dashboard, Telegram integration, token-based access control, traffic visibility, audit export, abuse detection and client ban management.
...
> Review all configuration before using it in production.
...
- NPMplus/reverse proxy friendly deployment.
...
Telegram Bot
  ...
  └── Local ProxyTor API
        ├── Status checks
        ├── Token retrieval/rotation
        ├── Ban/unban actions
        └── Abuse alerts
Components
Component	Default Port	Description
Tor SOCKS	9050/tcp	SOCKS5 proxy exposed to trusted clients
...
SQLite	Local file	Stores events, clients, traffic samples and bans
...
Publish the dashboard only through VPN, reverse proxy authentication or private access.
Use strong admin/viewer tokens.
Rotate tokens periodically.
Keep Telegram bot credentials private.
Never commit production tokens, real internal IPs or private domain names.
Repository Layout
proxytor-gateway/
├── README.md
├── LICENSE
